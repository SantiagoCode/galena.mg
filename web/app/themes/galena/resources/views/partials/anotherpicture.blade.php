<body>
    <div class="_galena2-background columns is-flex is-align-items-end p-6" data-cursor-img="@asset('images/galena_img_2.png')">

        <div class="_gallery column is-1 is-offset-1">
            <a href="#" class="_gallery is-capitalized has-text-light">gallery</a>
        </div>

        {{-- ADRIAN 2023 --}}
        <div class="_simonbolivar-2023 column is-3 is-6-touch is-offset-6 is-offset-4-touch" data-inertia data-inertia-reveal data-inertia-delay="600">
            <div class="_mini-container mb-6">
                <p class="_frase has-text-light is-capitalized is-size-3 is-size-5-touch mb-3">
                    "Lorem ipsum, dolor sit amet consectetur adipisicing elit excepturi."
                </p>
                <p class="about is-flex has-text-light">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-circle">
                        <circle cx="12" cy="12" r="10"></circle>
                    </svg>
                    <span class="mx-3">
                        <span class="has-text-light is-capitalized is-size-7 has-text-weight-bold is-block">adrian salvatori</span>
                        <span class="has-text-light is-capitalized is-size-7">CEO Movida Company™ </span>
                    </span>
                </p>
            </div>
        </div>

    </div>
</body>