...
/**
 * Class used to manage ProjectHuddle users via the REST API.
 *
 * @since 2.7.0
 *
 * @see   WP_REST_Controller
 */
class PH_REST_Users_Controller extends WP_REST_Controller
{
				...
				array(
					...
					'args'                => array(
						...
						'access_token' => array(
							'type'        => 'string',
							'default'     => '',
							'description' => __('Access send access token optionally if cookie is not stored.'),
						),
					),
				),
				'schema' => array($this, 'get_login_schema'),
			)
		);
		...
		register_rest_route(
			...
			array(
				array(
					...
					'permission_callback' => array($this, 'reset_permissions_check'),
					'args'                => array(
						'context' => $this->get_context_param(array('default' => 'view')),
					),
				),
	...
	public function get_items($request)
	{
		...
		// if current user can't list all names
		if (!current_user_can('list_users')) {
			// default to only those with published posts
			$prepared_args['has_published_posts'] = get_post_types(array('show_in_rest' => true), 'names');

			// if we have a project arg
			if (isset($registered['project'])) {
				// get the request
				$project =  !empty($request['project']) ? $request['project'] : 0;

				// use current post if project is not defined
				global $post;
				if (!$project  && is_a($post, 'WP_Post')) {
					$project = $post->ID;
				}

				// let user get all users if they are a project member
				if (!empty($project) && ph_user_is_member($project)) {
					unset($prepared_args['has_published_posts']);
				}

				// let them list users if they have a valid access token
				if ($project && $access = PH()->session->get('project_access')) {
					$token = isset($access[$project]) ? $access[$project] : false;
					$validator = new PH_Permissions_Controller($post->ID, '', $token, '', '');
		...
		// if we are querying by project, include only relevant user ids in query.
		if (isset($registered['project']) && !empty($request['project'])) {
			$ids = ph_get_project_member_ids($request['project']);

			$prepared_args['include'] = !empty($prepared_args['include']) ? $prepared_args['include'] : array();

			// always include author
			if ($author_id = get_post_field('post_author', $request['project'])) {
				$prepared_args['include'][] = $author_id;
			}

			// include only member user ids.
			if (is_array($prepared_args['include'])) {
				$prepared_args['include'] = array_merge($ids, $prepared_args['include']);
			} else {
				$prepared_args['include'] = array($ids);
			}
		}
		 ...
		 */
		$prepared_args = apply_filters('ph_rest_user_query', $prepared_args, $request);
	...
	public function get_item_permissions_check($request)
	{
		...
		// does user have edit permissions and is looking for edit context?
		if ('edit' === $request['context'] && !current_user_can('list_users')) {
		...
		// TODO: maybe only show if user is member of project
		return true;
	}
	...
	{
		// need a valid project_id
		if (!$project = get_post($request['project_id'])) {
			return new WP_Error('rest_invalid_project_id', __('You must provide a valid project id to create the user for that project.', 'project-huddle'), array('status' => $this->authorization_status_code()));
		}

		// if no access token is sent via api, check the user's session
		if (!$access_token = $request['access_token']) {
			$access       = PH()->session->get('project_access');
			$access_token = isset($access[$project->ID]) ? $access[$project->ID] : false;
		}

		// newup validatation controller
		$validator = new PH_Permissions_Controller($project->ID, $request['_signature'], $access_token, $request['email']);

		// check if visitor can access
		$has_access = $validator->visitor_can_access();

		// return permissions or WP_Error
		if (!is_wp_error($has_access) && $has_access) {
			return true;
		} else {
			return $has_access;
		}
	...
	public function create_item($request)
	{
		...
		$prepared_user = $this->prepare_item_for_database($request);

		// while username exists, append an additional number to make sure username is unique
		$count = 0;
		do {
			$unique_username = sanitize_user($prepared_user->user_login);
			if ($count) {
				$unique_username = $prepared_user->user_login . '-' . $count;
				$unique_username = sanitize_user($unique_username);
			}
			$count++;
		} while (username_exists($unique_username));

		// create user if it doesn't exist
		if (!email_exists(sanitize_email($request['email']))) {
			if (isset($request['password']) && $request['password']) {
				$password = $request['password'];
			} else {
				$password = wp_generate_password();
			}
			if( ! validate_username( $unique_username ) ) {
				return new WP_Error('invalid_user_name', __('This username is invalid because it uses illegal characters. Please enter a valid username.'));
			}

			/** This filter is documented in wp-includes/user.php */
			$illegal_logins = (array) apply_filters( 'illegal_user_logins', array() );

			if ( in_array( strtolower( $unique_username ), array_map( 'strtolower', $illegal_logins ), true ) ) {
				return new WP_Error( 'invalid_username', __( 'Sorry, that username is not allowed.' ) );
			}

			$user_id = wp_create_user($unique_username, $password, sanitize_email($request['email']));
			$created = false;
		} else {
			// force error
			$user_id = new WP_Error('existing_user_email', __('Sorry, that email address is already used!'));
		}
		...
		if (is_wp_error($user_id)) {
			...
			// bail for everything except if the user email already exists
			...
			// does project allow users to login without password?
			$can_login = ph_project_allow_login_without_password($project->ID, $request['_signature'], $request['email'], true);
			...
			if (!is_a($user, 'WP_User')) {
				return $user_id; // return original email already exists if user doesn't have access
			}
			...
			$can_login = true;
			// force display name and role
			wp_update_user(
				array(
					'ID'           => $user_id,
					'display_name' => esc_sql($prepared_user->user_login),
					'role'         => 'project_client', // force project client
				)
			);
		 ...
		 */
		if ($created) {
			do_action('ph_rest_insert_user', $user, $request, true);
			
			// if send pass link is enable
			$is_enable_pass_reset_link = get_option('ph_pass_reset_link');
			
			// if enable send the password reset link to the project client
			if( $is_enable_pass_reset_link ) {
				$creds['user_login'] = $request['username'];
				do_action('ph_sent_user_password_reset', $creds['user_login'], $request);
			}	
		}
		...
		$request['id']      = $user->ID;
		$request['context'] = 'edit'; // use edit context to return permissions
		$response           = $this->get_item($request);

		$data = $response->get_data();
		$data['refresh_token'] =  User::getById($user->ID)->getRefreshToken();
	...
	public function logout_permissions_check()
	{
	 ...
	 */
	function login_permissions_check($request)
	{
		return true; // let anyone sign in
	}
	 ...
	 * @param $request
	 *
	 * @return WP_User|WP_Error User or error loging in
	 */
	function login($request)
	{
		$creds = array(
			'user_login'    => $request['username'],
			'user_password' => addslashes( $request['password'] ),
			'remember'      => true,
		);
	 ...
	 * @return bool|WP_Error
	 */
	function reset_permissions_check($request)
	{
		return true; // let anyone sign in
	}
	...
	function suppression_permissions_check($request)
	{
		$user = $this->get_user($request['id']);
		if (is_wp_error($user)) {
			return $user;
		...
		if ($user->ID !== get_current_user_id()) {
			return new WP_Error('rest_user_invalid', __('Sorry, you are not allowed add or remove others from suppression lists.'), array('status' => rest_authorization_required_code()));
		}
	...
	public function prepare_item_for_response($user, $request)
	{
		...
		if (!empty($schema['properties']['roles'])) {
			...
			$data['user_role'] = translate_user_role( $wp_roles->roles[ $user->roles[0] ]['name'] );
		}
	...
	protected function prepare_item_for_database($request)
	{
		...
		if (isset($request['username']) && !empty($schema['properties']['username'])) {
			$prepared_user->user_login = rtrim($request['username']);
		}
	...
	/**
	 * Roll our on validate username function to prevent themes and other plugins from filtering
	 *
	 * @param $username
	 *
	 * @return boolean
	...
	protected function validate_username($username)
	{
		 ...
		 */
		return apply_filters('ph_validate_username', $valid, $username);
	}
	 ...
	 * @return string
	 */
	protected function sanitize_user($username, $strict = false)
	{
		 ...
		 */
		return apply_filters('ph_sanitize_user', $username, $raw_username, $strict);
	}
	...
	public function get_item_schema()
	{
		$schema = array(
			...
			'properties' => array(
				...
				// 'access_token' => array(
				// 	'description' => __( 'Pass an access token with a specific user' ),
				// 	'type'        => 'string',
				// 	'context'     => array(),
				// ),
				// 'project_id' => array(
				// 	'description' => __( 'Pass a project id for access requests.' ),
				// 	'type'        => 'integer',
				// 	'context'     => array( 'embed', 'view', 'edit' ),
				// 	'default' => 0
				// ),
	 ...
	 * @return array Item schema data.
	 */
	public function get_login_schema()
	{
		$schema = array(
			'$schema'    => 'http://json-schema.org/schema#',
			'title'      => 'user',
	...
	public function get_collection_params()
	{
		...
		$query_params['project'] = array(
			'description' => __('Limit result set to users who are members of a project.'),
			'type'        => 'integer',
		);

		$query_params['show_name'] = array(
			'default'     => false,
			'description' => __('Show the persons name even if they are logged in as themselves.'),
			'type'        => 'boolean',
		);
...
function ph_register_user_routes()
{
	$controller = new PH_REST_Users_Controller();
	$controller->register_routes();
}

add_action('rest_api_init', 'ph_register_user_routes');
