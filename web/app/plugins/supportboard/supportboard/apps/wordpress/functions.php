 ...
 * ==========================================================
 *
 * WordPress App main file. © 2017-2022 board.support. All rights reserved.
 *
 */
...
function sb_wp_get_user($user_id) {
    ...
    if (!sb_is_error($user_data) && isset($user_data['ID'])) {
        ...
        return ['id' => $user_id, 'first_name' => ucfirst(empty($user_data['first_name']) ? $user_data['user_login'] : $user_data['first_name']), 'last_name' => ucfirst(sb_isset($user_data, 'last_name', '')), 'email' => sb_isset($user_data, 'user_email'), 'password' => sb_isset($user_data, 'user_pass')];
    }
    return false;
}
    ...
    if (isset($wp_user['email'])) {
        ...
        if ($user_db === '') {
            ...
            if ($active_user) {
                ...
                if (defined('SB_WOOCOMMERCE')) sb_woocommerce_assign_orders($user_db['id'], $active_user['id']);
            }
            $wp_user = $user_db;
        }
    ...
    if (isset($_POST['type'])) {
        switch ($_POST['type']) {
            ...
            case 'wp-registration':
                ...
                if (is_wp_error($wp_user_id)) die($wp_user_id->get_error_message());
