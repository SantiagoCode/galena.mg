    ...
    var SBProfile = {
        ...
        populate: function (user, profile_area) {
            ...
            if (profile_area.hasClass('sb-profile-list-conversation') && SBChat.conversation) {
                ...
                if (!SBF.null(source)) {
                    code += this.profileRow('conversation-source', source == 'fb' ? 'Facebook' : (source == 'wa' ? 'WhatsApp' : (source == 'tm' ? 'Text message' : (source == 'ig' ? 'Instagram' : (source == 'tg' ? 'Telegram' : (source == 'tk' ? 'Tickets' : (source == 'wc' ? 'WeChat' : (source == 'em' ? 'Email' : (source == 'tw' ? 'Twitter' : (source == 'bm' ? 'Business Messages' : source))))))))), sb_('Source'));
                }
            }
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets', 'woocommerce': 'Woocommerce', 'ump': 'Ultimate Membership Pro', 'perfex': 'Perfex', 'whmcs': 'WHMCS', 'aecommerce': 'Active eCommerce', 'messenger': 'Messenger', 'whatsapp': 'WhatsApp', 'armember': 'ARMember', 'telegram': 'Telegram', 'wechat': 'WeChat', 'twitter': 'Twitter', 'zendesk': 'Zendesk', 'gbm': 'Google Business Messages' };
                let updates = false;
        ...
        */
        ...
        $(document).on('SBMessageSent', function (e, response) {
            ...
            if (SBApps.gbm.check(SBChat.conversation)) {
                SBApps.gbm.send(activeUser().getExtra('gbm-id').value, response.message, response.attachments, (response) => {
                    if (response[0] && 'error' in response[0]) {
                        dialog(message_part + ' Business Messenger: ' + response[0].error.message, 'info');
                    }
                });
            }
        ...
        */
        ...
        $(settings_area).on('click', '#whatsapp-twilio-btn .sb-btn, #sms-btn .sb-btn, #wechat-btn .sb-btn, #twitter-callback .sb-btn, #gbm-webhook .sb-btn', function (e) {
            let id = $(this).closest('[id]').attr('id');
            dialog(`<pre>${SB_URL + (id == 'sms-btn' ? '/inlude/api.php' : ('/apps/' + (id == 'wechat-btn' ? 'wechat' : (id == 'twitter-callback' ? 'twitter' : (id == 'gbm-webhook' ? 'gbm' : 'whatsapp'))) + '/post.php')) + cloudURL().replace('&', '?')}</pre>`, 'info');
            return false;
            e.preventDefault();
        });
