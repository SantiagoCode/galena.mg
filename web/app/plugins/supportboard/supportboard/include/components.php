 ...
 * ==========================================================
 *
 * Library of static html components for the admin area. This file must not be executed directly. © 2017-2022 board.support. All rights reserved.
 *
 */
...
function sb_component_admin() {
    ...
    $is_supervisor = sb_is_supervisor();
    $is_admin = $active_user && sb_is_agent($active_user, true, true) && !$is_supervisor;
    $sms = sb_get_multi_setting('sms', 'sms-user');
    $css_class = ($logged ? 'sb-admin' : 'sb-admin-start') . (sb_get_setting('rtl-admin') || ($is_cloud && defined('SB_CLOUD_DEFAULT_RTL')) ? ' sb-rtl' : '') . ($is_cloud ? ' sb-cloud' : '') . ($is_supervisor ? ' sb-supervisor' : '');
    if ($is_supervisor && !sb_get_multi_setting('supervisor', 'supervisor-send-message')) {
...
?>
    <div class="sb-main <?php echo $css_class ?>" style="opacity: 0">
        ...
        <div class="sb-header">
            <div class="sb-admin-nav">
                ...
                <div>
                    ...
                    <?php
                  if (($is_admin || sb_get_setting('admin-agents-users-area') || sb_get_multi_setting('agents', 'agents-users-area')) || ($is_supervisor && sb_get_multi_setting('supervisor','supervisor-users-area'))) echo '<a id="sb-users"><span>' . sb_('Users') . '</span></a>'; // temp delete sb_get_setting('admin-agents-users-area')
                  if ($is_admin || ($is_supervisor && sb_get_multi_setting('supervisor','supervisor-settings-area'))) echo '<a id="sb-settings"><span>' . sb_('Settings') . '</span></a>';
                  if (($is_admin && !sb_get_multi_setting('performance', 'performance-reports')) || ($is_supervisor && sb_get_multi_setting('supervisor','supervisor-reports-area'))) echo '<a id="sb-reports"><span>' . sb_('Reports') . '</span></a>';
                    ?>
                </div>
            </div>
            <div class="sb-admin-nav-right sb-menu-mobile">
                ...
                <div class="sb-mobile">
                    <?php if ($is_admin || sb_get_multi_setting('agents', 'agents-edit-user') || ($is_supervisor && sb_get_multi_setting('supervisor','supervisor-edit-user'))) echo '<a href="#" class="edit-profile">' . sb_('Edit profile') . '</a>'  . ($is_cloud ? ('<a href="#" data-value="account">' . sb_('Account') . '</a>') : '') . '<a href="#" class="sb-docs">' . sb_('Docs') . '</a><a href="#" class="sb-version">' . sb_('Updates') . '</a>' ?>
        ...
        <main>
            <div class="sb-active sb-area-conversations">
                <div class="sb-board">
                    ...
                    <div class="sb-conversation">
                        <div class="sb-top">
                            ...
                            <div class="sb-menu-mobile">
                                ...
                                <ul>
                                    ...
                                    <?php if ($is_admin || sb_get_setting('agents-delete') || sb_get_multi_setting('agents', 'agents-delete-conversations') || ($is_supervisor && sb_get_multi_setting('supervisor','supervisor-delete-conversation'))) echo '<li><a data-value="delete" class="sb-btn-icon sb-btn-red" data-sb-tooltip="' . sb_('Delete conversation') . '"><i class="sb-icon-delete"></i></a></li><li><a data-value="empty-trash" class="sb-btn-icon sb-btn-red" data-sb-tooltip="' . sb_('Empty trash') . '"><i class="sb-icon-delete"></i></a></li>'//temp delete  sb_get_setting('agents-delete') ?>
                                </ul>
                            </div>
                        ...
                        <?php if (sb_get_setting('chat-sound-admin') != 'n' || sb_get_setting('online-users-notification')) echo '<audio id="sb-audio" preload="auto"><source src="' . SB_URL . '/media/sound.mp3" type="audio/mpeg"></audio><audio id="sb-audio-out" preload="auto"><source src="' . SB_URL . '/media/sound-out.mp3" type="audio/mpeg"></audio>' ?>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="sb-menu-wide sb-menu-users">
                            ...
                            <ul>
                                ...
                                <?php if ($is_admin || sb_get_setting('admin-agents-tab') || sb_get_multi_setting('agents', 'agents-tab') || ($is_supervisor && sb_get_multi_setting('supervisor','supervisor-agents-tab'))) { echo '<li data-type="agent">' . sb_('Agents & Admins') . '</li>'; } // temp delete sb_get_setting('admin-agents-tab') ?>
                            </ul>
                        </div>
                        <div class="sb-menu-mobile">
                            <i class="sb-icon-menu"></i>
            ...
            <?php if ($is_admin || ($is_supervisor && sb_get_multi_setting('supervisor','supervisor-settings-area'))) { ?>
...
function sb_apps_panel() {
    $code = '';
    $collapse = sb_get_setting('collapse') ? ' sb-collapse' : '';
    $panels = [['SB_UMP', 'ump'], ['SB_WOOCOMMERCE', 'woocommerce'], ['SB_PERFEX', 'perfex'], ['SB_WHMCS', 'whmcs'], ['SB_AECOMMERCE', 'aecommerce'], ['SB_ARMEMBER', 'armember'], ['SB_ZENDESK', 'zendesk']];
...
function sb_conversations_filter() {
    ...
    $sources = [['em', 'Email', true], ['tk', 'Tickets', 'SB_TICKETS'], ['wa', 'WhatsApp', 'SB_WHATSAPP'], ['fb', 'Messenger', 'SB_MESSENGER'], ['ig', 'Instagram', 'SB_MESSENGER'], ['tg', 'Telegram', 'SB_TELEGRAM'], ['tw', 'Twitter', 'SB_TWITTER'], ['wc', 'WeChat', 'SB_WECHAT'], ['tm', 'Text message', true]];
    for ($i = 0; $i < $count; $i++) {
