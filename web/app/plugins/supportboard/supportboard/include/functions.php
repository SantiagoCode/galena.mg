 ...
 * ==========================================================
 *
 * Main PHP functions file. © 2017-2022 board.support. All rights reserved.
 *
 */

define('SB_VERSION', '3.5.1');
...
$sb_apps = ['dialogflow', 'slack', 'wordpress', 'tickets', 'woocommerce', 'ump', 'perfex', 'whmcs', 'aecommerce', 'messenger', 'whatsapp', 'armember', 'telegram', 'wechat', 'twitter', 'zendesk', 'gbm'];
...
function sb_get_cookie_login() {
    if (isset($_COOKIE['sb-login'])) {
        $response = json_decode(sb_encryption($_COOKIE['sb-login'], false), true);
...
function sb_is_supervisor() {
    return in_array(sb_get_active_user_ID(), explode(',', str_replace(' ', '', sb_get_multi_setting('supervisor', 'supervisor-id'))));
}
...
function sb_update_user_value($user_id, $slug, $value, $name = false) {
    $user_id = sb_db_escape($user_id, true);
    if (sb_isset(sb_get_active_user(), 'user_type') != 'admin' && ((sb_is_agent() && sb_isset(sb_db_get('SELECT user_type FROM sb_users WHERE id = ' . $user_id), 'user_type') == 'admin') || ($slug == 'user_type' && ($value == 'agent' || $value == 'admin')))) {
...
function sb_get_conversations($pagination = 0, $status_code = 0, $department = false, $source = false) {
    $status_code = $status_code == 0 ? 'AND C.status_code <> 3 AND C.status_code <> 4' : 'AND C.status_code = ' . sb_db_escape($status_code);
    $result = sb_db_get('SELECT A.*, B.user_type as message_user_type, C.title FROM sb_messages A, sb_users B, sb_conversations C WHERE B.id = A.user_id ' . $status_code . ' AND C.id = A.conversation_id' . ($source ? ' AND C.source = "' . sb_db_escape($source) . '"' : '') . (sb_get_agent_department() === false && $department ? ' AND C.department = ' . sb_db_escape($department, true) : '') . sb_routing_and_department_db('C') . ' AND A.id IN (SELECT max(id) FROM sb_messages WHERE message <> "" OR attachments <> "" GROUP BY conversation_id) GROUP BY conversation_id ORDER BY C.status_code DESC, A.id DESC LIMIT ' . (intval(sb_db_escape($pagination, true)) * 100) . ',100', false);
    if (isset($result) && is_array($result)) {
...
function sb_routing_and_department_db($table_name = 'sb_conversations') {
    ...
    return ($routing ? (' AND (' . $table_name. '.agent_id = ' . sb_get_active_user_ID() . ($routing_unassigned ? (' OR (' . $table_name . '.agent_id IS NULL OR ' . $table_name. '.agent_id = ""))') : ')')) : '') . ($department !== false ? ' AND ' . $table_name. '.department = ' . $department : '');
}
    ...
    if ($sender_id != -1) {
        ...
        if ($security || $user_id == sb_get_active_user_ID() || !empty($GLOBALS['SB_FORCE_ADMIN'])) {
            ...
            if ((!$queue_active || (!$is_sender_agent && !sb_agents_online())) && !$is_sender_bot && !$is_human_takeover) {
                ...
                if (($agent_check && (sb_get_setting('notify-agent-email') || sb_get_multi_setting('email-piping', 'email-piping-active'))) || ($user_check && !empty($user['email']) && sb_get_setting('notify-user-email') && (empty($conversation['source']) || in_array($conversation['source'], ['tk', 'em'])))) {
...
function sb_rich_value($value, $merge_fields = true, $tranlsate = true, $shortcodes = false) {
    ...
    return trim(preg_replace('/\s+/', ' ', $merge_fields ? sb_merge_fields($value) : $value));
}
...
function sb_get_user_language($user_id = false) {
    ...
    if (empty($SB_LANGUAGE)) {
        return strtolower(isset($_GET['lang']) ? $_GET['lang'] : (isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) ? substr($_SERVER['HTTP_ACCEPT_LANGUAGE'], 0, 2) : ''));
    }
...
function sb_email_send($to, $subject, $body, $sender_suffix = '') {
    ...
    if (!$settings['email-server-host'] && sb_is_cloud()) $settings = ['email-server-host' => CLOUD_SMTP_HOST, 'email-server-user' => CLOUD_SMTP_USERNAME, 'email-server-password' => CLOUD_SMTP_PASSWORD, 'email-server-from' => CLOUD_SMTP_SENDER, 'email-sender-name' => CLOUD_SMTP_SENDER_NAME, 'email-server-port' => CLOUD_SMTP_PORT];
                        ...
                        if (!in_array($id, $history)) {
                            ...
                            if ($sender && ($follow_up || !sb_is_agent($sender))) {
                                ...
                                if (strpos($message, 'quoted-printable')) {
                                    ...
                                    $encoding = isset($structure->parts) && count($structure->parts) ? $structure->parts[0]->encoding : ($structure->encoding ? $structure->encoding : false);
                                    if ($encoding) {
                                        switch ($encoding) {
                                            case 0:
                                            case 1:
...
function sb_email_piping_attachments($structure, &$inbox, &$email_number, $part_index = false) {
    ...
    if (isset($structure->parts) && count($structure->parts)) {
        for ($i = 0; $i < count($structure->parts); $i++) {
            ...
            if ($attachment) {
                $attachment_temp = imap_fetchbody($inbox, $email_number, (($part_index ? $part_index : $i) + 1) . '.' . ($i + 1));
                if ($part->encoding == 3) {
            ...
            if (property_exists($part, 'parts') && $part->parts) {
                return array_merge($attachments, sb_email_piping_attachments($part, $inbox, $email_number, $i));
            }
        }
    }
    return $attachments;
...
function sb_installation($details, $force = false) {
    ...
    if ($connection_check === true) {
        ...
        $db_respones['reports'] = $connection->query('CREATE TABLE IF NOT EXISTS sb_reports (id INT NOT NULL AUTO_INCREMENT, name VARCHAR(255) NOT NULL, value VARCHAR(255) NOT NULL, creation_time DATE NOT NULL, external_id INT, extra VARCHAR(255), PRIMARY KEY (id)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci');
...
function sb_updates_validation() {
    if (!isset($_COOKIE['sb-updates']) || $_COOKIE['sb-updates'] != SB_VERSION) {
        ...
        sb_db_query('ALTER TABLE sb_reports DROP INDEX IF EXISTS sb_reports_data_index');
...
function sb_messaging_platforms_send_message($message, $conversation, $message_id = false, $attachments = []) {
    ...
    switch ($conversation['source']) {
        ...
        case 'wa':
            return sb_whatsapp_send_message($platform_value ? $platform_value : sb_get_user_extra($user_id, 'phone'), $message, $attachments);
        case 'tg':
...
function sb_cloud_membership_validation() {
    ...
    echo '<script>var SB_CLOUD_FREE = ' . (empty($membership['id']) || $membership['id'] == 'free' ? 'true' : 'false') . '</script>';
    if (!$membership || !isset($membership['count']) || intval($membership['count']) > intval($membership['quota']) || (isset($membership['count_agents']) && isset($membership['quota_agents']) && intval($membership['count_agents']) > intval($membership['quota_agents'])) || ($membership['price'] != 0 && (!$expiration || time() > $expiration->getTimestamp()))) die('<script>document.location = "' . CLOUD_URL . '/account"</script>');
}
